...
var Location = function () {
  ...
  _createClass(Location, [{
        ...
        return _regeneratorRuntime.wrap(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                if (!(!_this.amapLoaded && _this.type === 'amap')) {
                  _context.next = 6;
                  break;
                }
              ...
              case 3:
                _this.initAMap();
                _context.next = 6;
                return _this.installGeoLocation();
              ...
              case 6:
                ...
                _this.success = true;
                _this.cityId = fitCity.id;
                _this.cityName = fitCity.name;
                _this.lng = Number(getCookie('fit-lng')) || null;
                _this.lat = Number(getCookie('fit-lat')) || null;
                return _context.abrupt('return', _this);

              case 15:
                return _context.abrupt('return', _this._getLocation());
        ...
        xhr.onreadystatechange = function () {
          if (xhr.status === 200) {
            if (xhr.readyState === 4) {
              ...
              if (res.code === 0) {
                _this2.cityName = res.data.regeo.city;
                _this2.cityId = res.data.regeo.city_id, _this2.province = res.data.regeo.province;
                _this2.provinceId = res.data.regeo.province_id;
                _this2.isOpenCity = res.data.regeo.is_open_city;
                resolve(true);
              } else {
    ...
    value: function amapLocation() {
      ...
      // 高德地图定位
      return new Promise(function (resolve, reject) {
        _this3.geolocation.getCurrentPosition(function (status, res) {
          if (status === 'complete') {
            resolve(res);
          } else {
            // localhost可以 ip不能绕过
    ...
    value: function appLoacation() {
      ...
      return new Promise(function (resolve, reject) {
        window.LeFitWebViewJavascriptBridge && window.LeFitWebViewJavascriptBridge.callHandler('getLocation', {
          ...
          _this4.address = result.address;
          // this.lat = result.latitude
          // this.lng = result.longitude
          _this4.selectCityId = result.selectCityId;
          _this4.selectCityName = result.selectCityName;
    ...
    value: function location2AddressAMap(cityName, location) {
      ...
      return new Promise(function (resolve, reject) {
        _this5.geocoder.getAddress(location, function (status, result) {
          if (status === 'complete' && result.info === 'OK') {
            _this5.address = result.regeocode && result.regeocode.formattedAddress;
            resolve(true);
          } else {
      ...
      return new Promise(function (resolve, reject) {
        JSONP({
          ...
          callback: function callback(data) {
            if (data.status === 0) {
              _this6.address = data.result.address;
              resolve(true);
            } else {
    ...
    value: function installGeoLocation() {
      ...
      return new Promise(function (resolve) {
        _this8.map.plugin('AMap.Geolocation', function () {
